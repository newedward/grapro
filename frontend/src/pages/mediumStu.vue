<template>
    ...
    :closable="false"
      ...
      <el-button v-if="!isnew" type="success" @click="dialogVisible = true" :disabled="haspassed">点击提交中期相关文件</el-button>
    <el-tooltip v-show="haspassed" effect="dark" content="您的中期报告已经存档，无法重复提交" placement="top-start">
 ...
 beforefileUpload(file) {
  ...
  handlefileSuccess(response, file, fileList) {
   ...
   // console.log(file.raw);写一条记录
    this.$http.post('/api/addRecordStu/',{'stuId':this.watchId,'workId':this.workid,'path':response['path'],'process':'M'},{emulateJSON:true})
      ...
      getstatus(){
            this.$http.post('/api/getStudentMediumProcess/',{'stuId':this.watchId},{emulateJSON:true})
              .then(function(response){
                 var res1 = JSON.parse(response.bodyText);
      ...
      getdata(){
        ...
        // 设置布尔变量，进行修改或新增
            this.$http.post('/api/getRecordTitle/',{'stuId':this.watchId,'process':'M'},{emulateJSON:true})
              .then(function(response){
                    var res1 = JSON.parse(response.bodyText);
                    if(res1['err_code'] ==0) {
                      if(res1['title'] == "null"){
