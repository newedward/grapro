<template>
    ...
    :closable="false"
      ...
      <el-button v-if="!isnew" type="success" @click="dialogVisible = true" :disabled="haspassed">点击提交结题相关文件</el-button>
    <el-tooltip v-show="haspassed" effect="dark" content="您的结题文件已经存档，无法重复提交" placement="top-start">
  ...
  :rows="8" v-model="introduction" placeholder="请输入提交说明（可以为空）"></el-input>
            ...
            <el-row >
              <el-divider content-position="left">支持.rar/.zip/.7z文件</el-divider>
              <el-upload
                :data="dataf"
                :headers="headerf"
                name = "work"
 ...
 beforefileUpload(file) {
  ...
  handlefileSuccess(response, file, fileList) {
   ...
   // console.log(file.raw);写一条记录
    this.$http.post('/api/addRecordStu/',{'stuId':this.watchId,'workId':this.workid,'path':response['path'],'process':'E'},{emulateJSON:true})
      ...
      getstatus(){
            this.$http.post('/api/getStudentEndProcess/',{'stuId':this.watchId},{emulateJSON:true})
              .then(function(response){
                 var res1 = JSON.parse(response.bodyText);
                this.status = res1["info"];
                 this.statusicon = res1["type"];
                 if (res1["statuscode"] >= 100 || res1["statuscode"] < 70){
                   this.haspassed = true;
                 }
              })
      },
      getdata(){
        ...
        // 设置布尔变量，进行修改或新增
            this.$http.post('/api/getRecordTitle/',{'stuId':this.watchId,'process':'E'},{emulateJSON:true})
              .then(function(response){
                    var res1 = JSON.parse(response.bodyText);
                    if(res1['err_code'] ==0) {
                      if(res1['title'] == "null"){
